...
export function AltoProducts() {
  const [altoProducts, setAltoProducts] = useState<ProductDTO[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let mounted = true;
    fetch("/api/products/alto")
      .then((r) => r.json())
      .then((data) => {
        if (!mounted) return;
        setAltoProducts(data?.data ?? []);
      })
      .catch(() => setAltoProducts([]))
      ...
      <div className="flex flex-col items-start gap-4 mb-12 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Premium accessories designed for Alto vehicles
          </p>
        </div>
        <Link
          href="/products?q=Alto"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {loading
          ...
          : altoProducts.map((product, idx) => (
              ...
              >
                <span className="absolute top-6 left-6 z-10 bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded">
                  ALTO
                </span>
